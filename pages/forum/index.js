...
const Forum = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch("/api/discussion-board");
  ...
  const handleLike = async (postId) => {
    try {
      ...
      const response = await fetch(`/api/discussion-board/${postId}/like`, {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (postId) => {
    try {
      const response = await fetch(`/api/discussion-board/${postId}`, {
        method: "DELETE",
      });
      const data = await response.json();
      if (response.ok) {
